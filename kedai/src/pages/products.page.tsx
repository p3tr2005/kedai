...
const getAllProducts = async (query: string | undefined) => {
  try {
    const resp = !query ? await fetch("http://localhost:3001/products") : await fetch(`http://localhost:3001/products?search=${query}`);
...
function ProductsPage() {
  ...
  return <main className="w-full h-screen px-10">
